...
to handle-user-strategy [input-string]
  ...
  ask players with [user-id = hubnet-message-source] [
    set strategy-string input-string
  ]
end
...
to execute-command [command]
  ...
  if command = "strategy-string" [handle-user-strategy hubnet-message]
end
...
- It is entirely possible for participants to change their strategy string mid-simulation as there isn’t an easy way to prevent them from doing so. Doing this causes quite a few errors so participants should be instructed to refrain from doing that.
